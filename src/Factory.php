...
class Factory
{
   ...
   * @param string  $type Which kind of adapter to build, if omitted will be
                          chosen automatically (memcache vs memcached)
   ...
   */
    public static function create(
        array $config,
        $type = null,
        ConfigService $config_service = null,
    ...
    public function __construct($type = null)
    {
        ...
        if (!empty($type)) {
            ...
            if (!$this->_isExtensionLoaded($this->_getExtensionName($type))) {
                throw new Exceptions\SystemRequirementException(
                    "Selected cache adapter not available: " . var_export($type, 1)
                );
            }
